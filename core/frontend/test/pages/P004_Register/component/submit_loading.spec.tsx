...
describe('P004_Register 组件测试 - 提交状态反馈', () => {
  it('提交期间按钮被禁用', async () => {
    const user = userEvent.setup()
    const registerMock: SpyInstance = vi.spyOn(api as any, 'register')
    registerMock.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve({} as any), 50)))
    render(
      <MemoryRouter>
        <RegisterPage />
      </MemoryRouter>
    )
    await user.type(screen.getByPlaceholderText('用户名设置成功后不可修改'), 'user_t')
    await user.type(screen.getByPlaceholderText('请输入姓名'), '李雷')
